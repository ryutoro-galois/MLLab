# 20180913_02_TreeModel_TuningParameter.R
#
# @date  : 2018/09/13(Thu.)
# @brief : TreeModel parameter tuning
...
targetModelMethodNm_withID <- paste0(targetModelMethodID, "_", targetModelMethodNm)
...
if(T){
  ...
  source("Rpgm/InitSettings_Titanic.R", encoding="utf-8")
}
...
if(F){
  ### チューニングパラメータ ###
  # http://topepo.github.io/caret/train-models-by-tag.html#Model_Tree
  # 
  # [1]: method="rpart"
...
#   - Tuning parameters: cp (Complexity Parameter)
if(T){
  lst_model_cv[["rpart"]] <- caret::train(form=formulaStr, data=dat, method="rpart", tuneLength=7, 
                                          trControl=trainControl(method='cv', number=5, selectionFunction='best'))
  print(lst_model_cv[["rpart"]])
}

# [2]: method="rpart2"
#   - Tuning parameters: maxdepth (Max Tree Depth)
if(T){
  lst_model_cv[["rpart2"]]  <- caret::train(form=formulaStr, data=dat, method="rpart2", tuneLength=7, 
                                            trControl = trainControl(method='cv', number=5, selectionFunction='best'))
  print(lst_model_cv[["rpart2"]])
}


# select final model
model_cv <- lst_model_cv[[targetModelMethodNm]]
model <- model_cv$finalModel 


# リーフ数(終端ノード数)
if(T){
  nodes <- as.numeric(row.names(model$frame))
  is.leaf <- (model$frame$var == "<leaf>")
  numOfLeafsInTree <- length(nodes[is.leaf])
  print(numOfLeafsInTree)
}
...
if(T)
{
  ...
  strTreeCondition <- paste0("caret_[", targetModelMethodNm_withID, "]")
...
if(T){
  ...
  # outFilePath
  outFileName <- paste("tree_varImp_", strTreeCondition, ".png", sep="")
  outFilePath <- paste(outputDir, outFileName, sep="")
...
if(T){
  ...
  datTrain_ext[, varname_NEW] <- as.numeric(predict(model_cv, dat, "prob")[,2])
  datValid_ext[, varname_NEW] <- as.numeric(predict(model_cv, datValid, "prob")[,2])
  datTest_ext[, varname_NEW] <- as.numeric(predict(model_cv, datTest, "prob")[,2])
}
...
if(T){
  ...
  outFileName <- paste0("output_[", statisticalMethodNm, "]_", strTreeCondition, "_cutoff_", cutoff, ".csv")
  outFilePath <- paste(outputDir, outFileName, sep="")
