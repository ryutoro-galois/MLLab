# 20180911_TreeModel_PostPruning.R
#
# @date : 2018/09/11(Tue.)
# @note : 決定木モデル(事後剪定; Post-Pruning)
...
if(T){
  ...
  # rpart with cp=0
  model_cp0 <- rpart(formulaStr, dat, method="class", parms=list(split=Split),
                     control=rpart.control(cp=0, maxdepth=MaxDepth_cp0, xval=10))
  # set
  lst_model[["model_cp0"]] <- model_cp0
  ...
  if(T){
    ...
    png(outFilePath, width=CNST_WIDTH, height=CNST_HEIGHT)
    par(xpd=NA)
    try({
      plotcp(model_cp0)
    },TRUE)
    dev.off()
    # set
    lstOutFilePath[["plotCP_cp0"]] <- outFilePath
  }
  
  # CPInfo_cp0
  CPInfo_cp0 <- model_cp0$cptable
  ...
  # get cp_min
  if(T){
    CPInfo_min <- CPInfo[which(CPInfo[,"type"] %in% "min"),]
    cp_min <- as.numeric(CPInfo_min["CP"])
    nsplit_min <- as.numeric(CPInfo_min["nsplit"])
  }
  
  # get cp_1se
  if(T){
    CPInfo_1se <- CPInfo[which(CPInfo[,"type"] %in% "1se"),]
    cp_1se <- as.numeric(CPInfo_1se["CP"])
    nsplit_1se <- as.numeric(CPInfo_1se["nsplit"])
  }
