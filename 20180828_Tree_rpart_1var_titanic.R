# 20180828_Tree_rpart_1var_titanic.R
#
# @date : 2018/08/28(Tue.)
# @note : 1変数決定木(rpart)
#
...
for(k in (1:n))
{
  varname <- targetVariablesList[k]
  #strVarname <- paste("[", varname, "]", sep="")
  strVarname <- paste(varname, sep="")
  
  # init
  datSample <- datSample_init[,c(targetName, varname)]
  
  str_k <- paste("00", k, sep="")
  str_k <- substr(str_k, nchar(str_k)-2, nchar(str_k))
  str_k_label <- paste(varname, collapse="_")
  str_k_label <- paste("ptn", str_k, "_", str_k_label, sep="")
  
  varnameList_k_kakko <- paste("[", varname, "]", sep="")
  str_a1 <- paste("ptn", str_k, " : [", targetName, "] ～ ", paste(varnameList_k_kakko, collapse="+"), sep="")
  if(T){
    forPrint_k <- paste("[", str_k, "/", n, "] :  (", varname, ")", sep="")
    print(forPrint_k)
  }
  ...
  if(T){
    ...
    if(F){
      model <- rpart(formulaStr, datSample, method="class", parms=list(prior=Prior, split=Split),
  ...
  if(isOutputPNG == T)
  {
    ...
    if(MinBucketTypeID == 1) {
      outFileName <- paste("tree_1var_rpart.plot_ptn", str_k, "_", strVarname, "_", MaxDepth, "depth_", MinBucket, ".png", sep="")
    } else if(MinBucketTypeID == 2) {
      outFileName <- paste("tree_1var_rpart.plot_ptn", str_k, "_", strVarname, "_", MaxDepth, "depth_", strMinBucketRate, ".png", sep="")
    }
    outFilePath <- paste(outputDir, outFileName, sep="")
    #set
    lst_png_rpart_1var[[k]] <- outFilePath
...
if(T){
  ...
  outFileName <- paste("treeRules_rpart_AllPtn_1var_[", targetName, "]_", MaxDepth, "depth_", 
...
num_lst_png_rpart_1var <- length(lst_png_rpart_1var)
